import React, { useEffect } from 'react'
import "../About/about.css"
import teamimg from "../../assets/images/lampwithlady.png"
import about_img from "../../assets/images/lampwithlady.png"
import monisha from "../../assets/images/saurabh.webp"
import ranu from "../../assets/images/saurabh.webp"
import prerna from "../../assets/images/saurabh.webp"
import rashi from "../../assets/images/saurabh.webp"
import anumam from "../../assets/images/saurabh.webp"
import saurabhsir from "../../assets/images/saurabh.webp"
import saurabhfather from "../../assets/images/saurabh.webp"
import Navbar from '../Navbar/navbar'
<img src={earthdot} title="earthdot" alt="earthdot" />
import earthdot from "../../assets/images/blackearth.webp"
import AOS from 'aos'
import Footer from '../Footer/footer';

const about = () => {

  useEffect(()=>{
    window.onload=()=>{
      AOS.init({duration:1000})
    }
  },[])
  useEffect(()=>{
    const wrap =  document.querySelector('.wrapper')
    const side = document.getElementById('mySidenav');
    wrap.addEventListener('click',function(){
      if (side.style.width >= '200px') {
        side.style.width = '0px';
      }
    else{
      side.style.width == '300px'
    }
    })
  })
  return (
    <>
<Navbar/>
    <div className="wrapper">
      <div className="mainpscon">
    <div className="row row1 ">
                <div className="col-md-4 firstCol">
                    <h1 className="aboutHeading">About</h1>
                    <div className="button1">
                        <a href="">Home<i class="fa-solid fa-angle-right"></i><span>About Us</span></a>
                    </div>
                </div>
                <div className="col-md-6 secondCol">
        
      <img className="image img-fluid" src={about_img} alt="oneklick-contact-img"  title='oneklick-contact-img' />
                </div>
            </div>
<div
  class="pscon12 about-3_area about-3space about-3plr about-3_plr-2"
>
  <div className="container-fluid">
    <div className="row align-items-center justify-content-center">
      <div className=" col-md-5 mb-70 mainimagesection text-end">
        <img src={teamimg} alt="oneklick-team" width={"100%"} title="oneklick-team" className='img-fluid'   style={{borderRadius:'20px'}} data-aos="fade-left"
              data-aos-offset="200"
              data-aos-easing="zoom-out"
              data-aos-duration="500"/>

      </div>
      <div className="col-xl-5  col-lg-6 col-md-12 mb-70  div2" data-aos="fade-right"
              data-aos-offset="300"
              data-aos-easing="ease-in-sine"
              data-aos-duration="500">
        <div className="maincontent">
          <div class="about-3__section-title pb-10">
            <h2 class="section-blue-subtitle char-anim"> About OneKlick Techno Renewable</h2>
            <h3 class="section-title1 section-title char-anim">
             Authorized Distributor of  <i>Jinko Solar </i>, <i>Solex</i>, <i>Novasys Green</i> & <i>Axitec</i><br />
            </h3>
          </div>
          <div className="para">
            <div className="para1 pb-7">
              <p className="paracontent" style={{textTransform:'capitalize'}}>
              Oneklick Techno Renewable is a pan India authorized distributor of solar modules from leading brands like Jinko Solar, Solex, Axitec, and Novasys.  We provide our customers with three key features:
              </p>
            </div>
            <div className="points">
              <ul>
                <li>
                  <i className="fal fa-check-square"></i>
                  <span style={{textTransform:'capitalize'}}>
                  quality products
                  </span>
                </li>
                <li>
                  <i className="fal fa-check-square"></i>
                  <span style={{textTransform:'capitalize'}}>
                  operational efficiency 
                  </span>
                </li>
                <li>
                  <i className="fal fa-check-square"></i>
                  <span style={{textTransform:'capitalize'}}>
                  total commitment
                  </span>
                </li>
                
              </ul>
            </div>
          </div>
        </div>
      </div>
    </div>
  </div>
</div>

<div className="feature__section-box text-center d-flex justify-content-center" style={{marginTop:'50px'}} data-aos="fade-up" data-aos-offset="200"
              data-aos-easing="ease-in-sine"
              data-aos-duration="400" data-aos-once='false'>
            <div className="col-md-8">
          <p className="section-title title-anim wow animate__fadeInUp">
          Our Customer Satisfaction Reflects Everything About Us And Makes Us The Best Solar Panel Distributor Company In India.
          </p>
          </div>
        </div>

<div className="row ps-cardrow" data-aos="fade-up" data-aos-offset="200"
              data-aos-easing="ease-in-sine"
              data-aos-duration="400" data-aos-once='false' data-aos-delay='200'  >
  <div className="col-md-2 pscol" >
    <div className="pscardimg pscardimg1">
      <img
        src="data:image/png;base64,"
        alt="card1"
        title="card1"
      />
    </div>

    <h4 className="h4ps">Quality</h4>
  </div>
  <div className="col-md-2 pscol">
    <div className="pscardimg pscardimg2">
      <img
        src="data:image/png;base64,"
        alt="card2"
        title="card2"
      />
    </div>
    <h4 className="h4ps">Trust</h4>
  </div>
  <div className="col-md-2 pscol">
    <div className="pscardimg pscardimg3">
      <img
        src="data:image/png;base64,"
        alt="card3"
        title="card3"
      />
    </div>
    <h4 className="h4ps">Affordable</h4>
  </div>
  <div className="col-md-2 pscol">
    <div className="pscardimg pscardimg4">
      <img
        src="data:image/png;base64,"
        alt="card4"
        title="card4"
      />
    </div>
    <h4 className="h4ps">Sustainability</h4>
  </div>
  <div className="col-md-2 pscol">
    <div className="pscardimg pscardimg5">
      <img
        src="data:image/png;base64,"
        alt="card5"
        title="card5"
      />
    </div>
    <h4 className="h4ps"> Communication</h4>
  </div>
</div>
<div class="team_area teamspace-2 team_plr grey-bg">
  <div className="container-fluid team">
    <div className="row align-items-end justify-content-center">
      <div className="col-xl-12 col-lg-6 col-md-9 col-sm-10" data-aos="fade-up" data-aos-offset="200"
              data-aos-easing="ease-in-sine"
              data-aos-duration="400" data-aos-once='false'>
        <div className="feature__section-box text-center">
          <p className="section-blue-subtitle title-anim wow animate__fadeInUp" style={{fontSize:'20px'}}>
          Meet Our Stellar Members in Action!
          </p>
          <h3 className="section-title title-anim wow animate__fadeInUp">
          Behind Every Achievement <br/>
          Lies a Strong Team
          </h3>
        </div>
      </div>
    </div>
    <div className="row psrow d-flex justify-content-center" data-aos="fade-up" data-aos-offset="200"
              data-aos-easing="ease-in-sine"
              data-aos-duration="400" data-aos-once='false' data-aos-delay='150'>
      <div className="col-xl-3 col-lg-4 col-md-6 col-sm-6 padding-6 wow animate__fadeInUp">
        <div className="container23">
          <img
            src={saurabhfather}
            alt="suresh-oneklick"
            className="image"
            title="suresh-oneklick"
          />
          <div className="ps-overlay">
            <div className="text"></div>
          </div>
        </div>
        <div
          style={{ textAlign: "center" }}
          className="team__author-info"
        >
          <h5 className="team__author-name">
            <a href="/react/sorex/about-me">Suresh Sethi</a>
          </h5>
          <span>Founder</span>
        </div>
      </div>
      <div className="col-xl-3 col-lg-4 col-md-6 col-sm-6 padding-6 wow animate__fadeInUp">

        <div className="container23">
          <img
            src={saurabhsir}
            title="saurabh-oneklick"
            alt="saurabh-oneklick"
            className="image"
          />
          <div className="ps-overlay">
            <div className="text"></div>
          </div>
        </div>
        <div
          style={{ textAlign: "center" }}
          className="team__author-info"
        >
          <h5 className="team__author-name">
            <a href="/react/sorex/about-me">Saurabh Sethi </a>
          </h5>
          <span>Business Developer Head</span>
        </div>
      </div>
      <div className="col-xl-3 col-lg-4 col-md-6 col-sm-6 padding-6 wow animate__fadeInUp">

        <div className="container23">
          <img
            src={anumam}
            alt="anu-oneklick"
            title="anu-oneklick"
            className="image"
          />
          <div className="ps-overlay">
            <div className="text"></div>
          </div>
        </div>
        <div
          style={{ textAlign: "center" }}
          className="team__author-info"
        >
          <h5 className="team__author-name">
            <a href="/react/sorex/about-me">Anupriya Sethi</a>
          </h5>
          <span>Digital Marketing Partner</span>
        </div>
      </div>
    </div>
    <div className="row psrow" data-aos="fade-up" data-aos-offset="200"
              data-aos-easing="ease-in-sine"
              data-aos-duration="400" data-aos-once='false' data-aos-delay='200'>

      
      <div className="col-xl-3 col-lg-4 col-md-6 col-sm-6 padding-6 wow animate__fadeInUp" >

        <div className="container23">
          <img
            src={monisha}
            alt="monisha-oneklick"
            className="image"
            title="monisha-oneklick"
          />
          <div className="ps-overlay">
            <div className="text"></div>
          </div>
        </div>
        <div
          style={{ textAlign: "center" }}
          className="team__author-info"
        >
          <h5 className="team__author-name">
            <a href="/react/sorex/about-me">Monisha Agarwal</a>
          </h5>
          <span>Regional Head</span>
        </div>
      </div>
      <div className="col-xl-3 col-lg-4 col-md-6 col-sm-6 padding-6 wow animate__fadeInUp">

        <div class="container23">
          <img
            src={rashi}
            alt="rashi-oneklick"
            title="rashi-oneklick"
            className="image"
          />
          <div class="ps-overlay">
            <div class="text"></div>
          </div>
        </div>
        <div
          style={{ textAlign: "center" }}
          className="team__author-info"
        >
          <h5 className="team__author-name">
            <a href="/react/sorex/about-me">Rashi Arora</a>
          </h5>
          <span>Regional Head</span>
        </div>
      </div>
      <div className="col-xl-3 col-lg-4 col-md-6 col-sm-6 padding-6 wow animate__fadeInUp">

        <div class="container23">
          <img
            src={ranu}
            alt="ranu-oneklick"
            class="image"
            title="ranu-oneklick"
          />
          <div class="ps-overlay">
            <div class="text"></div>
          </div>
        </div>
        <div
          style={{ textAlign: "center" }}
          className="team__author-info"
        >
          <h5 className="team__author-name">
            <a href="/react/sorex/about-me">Ranu Khandelwal</a>
          </h5>
          <span>Marketing Executive</span>
        </div>
      </div>
      <div className="col-xl-3 col-lg-4 col-md-6 col-sm-6 padding-6 wow animate__fadeInUp">
        
        <div class="container23">
          <img
            src={prerna}
            alt="prerna-oneklick" 
            title="prerna-oneklick"
            class="image"
          />
          <div class="ps-overlay">
            <div class="text"></div>
          </div>
        </div>
        <div
          style={{ textAlign: "center" }}
          className="team__author-info"
        >
          <h5 className="team__author-name">
            <a href="/react/sorex/about-me">Prerna Khatri</a>
          </h5>
          <span>Marketing Executive</span>
        </div>
      </div>
   
    </div>
  </div>
</div>

            
<div className="feature__section-box text-center d-flex justify-content-center" style={{marginTop:'0px'}}>
            <div className="col-md-8" >
          <h3 className="section-title title-anim wow animate__fadeInUp" ata-aos-easing="ease-in-sine" data-aos="fade-up"
              data-aos-duration="400" data-aos-once='false' >
              OneKlick Core Values
          </h3>
          </div>
        </div>

<div className="work-3_area work-3inner-wrapper work-3plr work-3_space">
  <div className="container-fluid" style={{ marginTop: "50px" }} data-aos="fade-up" data-aos-offset="200"
              data-aos-easing="ease-in-sine"
              data-aos-duration="400" data-aos-once='false' >
    <div className="row row-cols-xl-3 row-cols-md-2 " ata-aos-easing="ease-in-sine"
              data-aos-duration="400" data-aos-once='false' data-aos-delay='200'>
      <div className="col col-6 col-12 wow animate__fadeInUp">
        <div className="work-3__wrapper inner-wrapper-1 mb-30">
          <div className="work-3__item1">
            <div className="work-3__content  align-items-center mb-15">
              <div className="work-3__icon icon-color-1">
                <span>
                  <i class="fa-solid fa-thumbs-up"></i>
                </span>
              </div>

              <div className="work-3__title">
                <h4 className="work-3__section-title">
                  <a href="#" style={{ lineHeight: "30px" }}>
                    Highest Quality Solar Products At Cost Effective Price
                  </a>
                </h4>
              </div>
            </div>
            <div className="work-3__right-text">
              <p>
                We know that it can be hard to find the right products for
                your projects from such wide variety of choices,
                especially when you need quality and price both. That’s
                why we make sure that we advise you best suited product
                which are always delivered 100% in time and at an
                affordable price—and we’re committed to making it happen.
              </p>
            </div>
          </div>
        </div>
      </div>
      <div class="col col-6 col-12 wow animate__fadeInUp">
        <div class="work-3__wrapper inner-wrapper-2 mb-30">
          <div class="work-3__item2">
            <div class="work-3__content  align-items-center mb-15">
              <div class="work-3__icon icon-color-2">
                <span>
                  <i class="fa-solid fa-user-group"></i>
                </span>
              </div>

              <div class="work-3__title">
                <h4 class="work-3__section-title">
                  <a href="#"> Professional and Enthusiastic Team</a>
                </h4>
              </div>
            </div>
            <div class="work-3__right-text">
              <p>
              We strive to stay on top of new technologies and techniques so that we can deliver the best possible outcomes.  Our commitment to customer satisfaction, technical expertise, and industry-leading products position them as a reliable and trusted partner in solar renewable energy. Our team members are always willing to help — from advice on best suited solar products on your projects to technical support.
              </p>
            </div>
          </div>
        </div>
      </div>
      <div class="col col-6 col-12 wow animate__fadeInUp">
        <div class="work-3__wrapper inner-wrapper-3 mb-30">
          <div class="work-3__item3">
            <div class="work-3__content  align-items-center mb-15">
              <div class="work-3__icon icon-color-3">
                <span>
                  <i class="fa-solid fa-user-check"></i>
                </span>
              </div>

              <div class="work-3__title">
                <h4 class="work-3__section-title">
                  <a href="#">Reliability</a>
                </h4>
              </div>
            </div>
            <div class="work-3__right-text">
              <p>
              OneKlick Techno Renewable is committed to quality so we deliver whatever you want, whenever you want and wherever you want.  We provide reliable and sustainable solar energy solutions, which contribute to reducing carbon emissions and promoting a greener future. We are available 24/7/365 to answer any questions you may have.
              </p>
            </div>
          </div>
        </div>
      </div>
    </div>
  </div>
  <div className="container-fluid container-fluid-second " data-aos="fade-up" data-aos-offset="200"
              data-aos-easing="ease-in-sine"
              data-aos-duration="400" data-aos-once='false' data-aos-delay='200'>
  <div className="row row-cols-xl-3 row-cols-md-2  d-flex justify-content-center">
  <div class="col col-12 wow animate__fadeInUp  ">
        <div class="work-3__wrapper inner-wrapper-4 mb-30">
          <div class="work-3__item4">
            <div class="work-3__content  align-items-center mb-15">
              <div class="work-3__icon icon-color-4">
                <span>
                  <i class="fa-regular fa-coins"></i>
                </span>
              </div>

              <div class="work-3__title">
                <h4 class="work-3__section-title">
                  <a href="#">Premium Range</a>
                </h4>
              </div>
            </div>
            <div class="work-3__right-text">
              <p>
              From World’s most sold Solar module to German standard practices. We make sure that we deliver you the best solar modules for your home or business office in India.
              </p>
            </div>
            </div>
          </div>
        </div> 
  <div class="col col-12 wow animate__fadeInUp  ">
        <div class="work-3__wrapper inner-wrapper-4 mb-30">
          <div class="work-3__item5">
            <div class="work-3__content  align-items-center mb-15">
              <div class="work-3__icon icon-color-4">
                <span>
                  <i class="fa-sharp fa-solid fa-magnifying-glass"></i>
                </span>
              </div>

              <div class="work-3__title">
                <h4 class="work-3__section-title">
                  <a href="#">Transparency</a>
                </h4>
              </div>
            </div>
            <div class="work-3__right-text">
              <p>
                OneKlick ensures that our customers receive accurate
                information and all the relevant details that they need in
                order to find the product they are looking for.
              </p>
            </div>
            </div>
          </div>
        </div>
 
        
      </div>
  </div>
  <div className="server-wide-container">
    <div className="col-md-10 country-server-container "  data-aos="fade-up"
              data-aos-offset="300"
              data-aos-easing="ease-in-sine"
              data-aos-duration="500">
        <div className="col-md-10 country-second-container">
          <div className="row wrap-cotainer">
            {/* <div className="country-section-box col-md-12">
              <p className="country-title"> We Are Best</p>
              <h3 className="country-subtitle ">Counter Industries Server</h3>
            </div> */}
            <div className="earth-dot-one ">
              <img src={earthdot} title="earthdot" alt="earthdot" />
            </div>
            <div className="earth-dot-second ">
              <img src={earthdot} title="earthdot" alt="earthdot" />
            </div>
            
          <div className="col-md-8 d-flex justify-content-start align-items-center">
              <p className='section-title text-light m-0' >Our Business Service Apps of Agency.</p>
            </div>
            <div className="col-md-4 d-flex justify-content-center align-items-center">
              <h1  className='text-light'>Empty Array</h1>
            </div>
            
          </div>
          
          
          
        </div>
        </div>
    </div>

  </div>

  


    </div>
    <Footer/>


    </div>
    </>
  )
}

export default about
